...
if ($(window).width() <= 1024) {
  dampingValue = 0.35;        // still low on mobile
}
    ...
    .then((blogs) => {
      blogs.forEach((blog, index) => {
        ...
        const categories = blog.categories
          .map(
            (cat) =>
              `<a class="category" href="#" rel="category tag"><span>${cat}</span></a>`
          )
          .join("");
        ...
        document
          ...
          .forEach(a => {
            a.addEventListener('click', e => {
              e.preventDefault();
              document.querySelectorAll('.portfolio_filter a')
